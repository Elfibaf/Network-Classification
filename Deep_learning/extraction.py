...
def load_dataset(filename):
    ...
    labels_one_hot = np.zeros((len(labels),166), dtype = 'i')
    for i in range(len(labels)):
        labels_one_hot[i][dict_labels[labels[i]]] = 1
    
    return DataSet(nparray.astype(np.float),labels_one_hot)





#foo = load_dataset('../Data/Caida/data_caida_original.arff')
#print(foo.labels[5222])
